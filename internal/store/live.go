...
func (s *Store) RecordSyncAttempt(ctx context.Context, attemptedAt time.Time) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO sync_state(source_name, last_attempt_at, result)
		VALUES (?, ?, 'running')
		ON CONFLICT(source_name) DO UPDATE SET
			last_attempt_at = excluded.last_attempt_at,
			result = excluded.result`, syncSourceName, formatTime(attemptedAt.UTC()))
	if err != nil {
...
func (s *Store) RecordSyncSuccess(ctx context.Context, etag, lastModified string, succeededAt time.Time, result string) error {
	_, err := s.db.ExecContext(ctx, `
...
func (s *Store) RecordSyncFailure(ctx context.Context, failedAt time.Time, syncError error) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO sync_state(source_name, last_attempt_at, result)
		VALUES (?, ?, ?)
		ON CONFLICT(source_name) DO UPDATE SET
			last_attempt_at = excluded.last_attempt_at,
			result = excluded.result`, syncSourceName, formatTime(failedAt.UTC()), "error: "+sanitizedError(syncError))
	if err != nil {
