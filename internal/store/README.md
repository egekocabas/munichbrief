...
- `presentation.go`: public/review visibility rules and localized presentation
  queries.
- `pipeline_types.go`, `pipeline_settings.go`, and `translation_settings.go`:
  stable pipeline values, canonical preferences, and per-language
...
Pipeline cycles freeze their ordered steps, prompt versions, models, adapters,
and target incident IDs when activated. Jobs are claimed and completed transactionally;
