...
func TestPipelineHistoryCombinesCanonicalAndPostProcessingJobsWithStableCursors(t *testing.T) {
	...
	translation, found, err := database.ClaimPostProcessingJob(ctx, "translation", testPostProcessingContract("en", translationPlan.PromptVersion, []string{"title", "summary"}, "title_de", "summary_de"), PostProcessingClaimOptions{}, now)
	if err != nil || !found {
	...
	verification, found, err := database.ClaimPostProcessingJob(ctx, "category_verification", testPostProcessingContract("default", verificationPlan.PromptVersion, []string{"is_correct", "corrected_category"}, "title_de", "summary_de", "category"), PostProcessingClaimOptions{}, now)
	if err != nil || !found {
