...
func TestPipelineFreezesTargetsGroupsStepsAndStartsNextCycleImmediately(t *testing.T) {
	...
	translations := 0
	for {
		job, found, err := database.ClaimPostProcessingJob(ctx, "translation", true, nil, now)
		if err != nil {
			t.Fatal(err)
		}
		if !found {
...
func TestManualCyclesCoalesceExactDuplicatesAndSupersedeQueuedAutomaticWork(t *testing.T) {
	...
	separate, err := database.CreateManualPipelineCycle(ctx, "fixture", different, postPlans, nil, true, now.Add(3*time.Second))
