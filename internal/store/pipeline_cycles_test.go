...
func TestPipelineFreezesTargetsGroupsStepsAndStartsNextCycleImmediately(t *testing.T) {
	...
	translations := 0
	for {
		job, found, err := database.ClaimPostProcessingJob(ctx, "translation", testPostProcessingContract("en", "incident-translation-en-v1", []string{"title", "summary"}, "title_de", "summary_de"), PostProcessingClaimOptions{AllowScheduled: true}, now)
		if err != nil {
			t.Fatal(err)
		}
		if !found {
