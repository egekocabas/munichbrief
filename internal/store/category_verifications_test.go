...
func TestCategoryVerificationIsIndependentAuditableAndRunScoped(t *testing.T) {
	...
	if queued, err := database.RequeueAllCategoryVerifications(ctx, "fixture", CategoryVerificationPlan{PromptVersion: plan.PromptVersion, Model: "verify:all"}, now.Add(6*time.Minute)); err != nil || queued != 1 {
