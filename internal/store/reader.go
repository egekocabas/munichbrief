...
func (s *Store) installReaderIndex(ctx context.Context) error {
 ...
 FROM incidents i JOIN source_documents d ON d.id=i.source_document_id CROSS JOIN (` + strings.Join(languages, " UNION ALL ") + `) l WHERE ` + ready + `)`
	if _, err = tx.ExecContext(ctx, "DROP VIEW IF EXISTS reader_projection"); err != nil {
		return err
	}
	if _, err = tx.ExecContext(ctx, projection); err != nil {
		return fmt.Errorf("reader projection: %w", err)
	}
	columns := "incident_id,language,run_id,title,summary,area,category,assistance,event_date,event_time,day_part,published_at,published_date,position,number,time_group,search_text"
	refresh := func(ids string) string {
		return "DELETE FROM reader_documents WHERE incident_id IN (" + ids + "); INSERT INTO reader_documents(" + columns + ") SELECT " + columns + " FROM reader_projection WHERE incident_id IN (" + ids + ");"
	}
	for _, spec := range []struct{ table, ids string }{
		...
		for _, event := range []string{"INSERT", "UPDATE", "DELETE"} {
			...
			if _, err = tx.ExecContext(ctx, "CREATE TRIGGER "+name+" AFTER "+triggerEvent+" ON "+spec.table+when+" BEGIN "+refresh(ids)+" END"); err != nil {
				return fmt.Errorf("reader trigger: %w", err)
			}
		}
	}
	if _, err = tx.ExecContext(ctx, "DELETE FROM reader_documents; INSERT INTO reader_documents("+columns+") SELECT "+columns+" FROM reader_projection"); err != nil {
