...
func TestHyMT2NativeAdapterAddsOnlyConfiguredTargetGuidance(t *testing.T) {
	...
	for _, expected := range []string{"‘Verkehrspolizei’", "never public-transport police", "‘hospitalisé(e)’", "‘présomption d’innocence’", "‘condamnation définitive’", "never merely until a final decision", "‘Scheibe’", "‘vitre’", "‘Windschutzscheibe’", "never infer ‘pare-brise’", "‘an der [street]’", "never change it to nearby", "‘près de’ or ‘à proximité de’"} {
	...
	for _, expected := range []string{"REGOLA PRIORITARIA PER I TITOLI", "verbo italiano coniugato", "‘flieht’", "‘fugge’ o ‘si dà alla fuga’", "non scrivere mai l’agrammaticale ‘fuga dalla polizia’", "verifica mentalmente", "italiano standard naturale", "‘Scheibe’", "‘vetro’ o ‘finestrino’", "‘presunzione d’innocenza’", "‘condanna definitiva’ o ‘condanna passata in giudicato’", "mai con la generica ‘sentenza definitiva’", "può anche assolvere", "le indagini in corso restano in corso", "‘Überprüfung’ o ‘Kontrolle’", "non una ‘perquisizione’", "non consumo accertato"} {
