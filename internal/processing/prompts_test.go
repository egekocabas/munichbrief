...
func TestRegisteredTranslationPromptsUseOnePlaceholderContract(t *testing.T) {
	...
	for _, translation := range RegisteredTranslations() {
		...
		for _, expected := range []string{
			...
			`__MB_PLACE_####__`, "same token can intentionally occur more than once", "Copy every token occurrence exactly", "apostrophe-delimited grammatical suffix", "Preserve Markdown syntax and link URLs exactly",
		} {
