...
var gemma4LanguageNames = map[string]string{
	"de": "German", "bs": "Bosnian", "hi": "Hindi",
	"hr": "Croatian", "ru": "Russian", "es": "Spanish", "it": "Italian",
}

var madlad400LanguageTokens = map[string]string{
	"bs": "bs", "hi": "hi", "hr": "hr", "ru": "ru",
}
...
var (
	...
	towerPlusLanguageGuidance = map[string]string{
		"hi": "Use natural standard Hindi in Devanagari and output only the translation, with no explanation or note. जर्मन 'soll ... haben' हमेशा आरोप बताता है: उसका अनुवाद 'X पर Y करने का आरोप है' या 'X ने कथित तौर पर Y किया' जैसे स्पष्ट आरोप के रूप में करो। केवल 'पुलिस के अनुसार' लिखना पर्याप्त नहीं है और ऐसे वाक्य में 'X ने Y किया' जैसा तथ्यात्मक रूप मत लिखो। Preserve approximate times with wording such as लगभग or करीब. Preserve subject, object, and every causal relationship exactly. Translate 'Scheibe eines Fahrzeugs' as a vehicle window or pane, such as वाहन की खिड़की or वाहन का शीशा, never as an unrelated object. Translate 'zuständige Dienststelle' as the relevant police station or police office, not an individual officer's office.",
		"ru": "Пиши только один естественный русский перевод без пояснений, заголовков и вариантов. Немецкое 'soll ... haben' обозначает обвинение: связывай 'предположительно' или 'якобы' с самим предполагаемым действием, не выдавая его за установленный факт. Перед каждым временем с немецким 'gegen' пиши 'около', а не точное 'в'; не заменяй этот маркер словом об обвинении. 'Polizeieinsatz' переводи нейтрально как 'полицейская операция' или 'действия полиции', не как рейд или захват. 'widerstandslos festgenommen' означает 'задержан без сопротивления', а не безопасно задержан. Нейтральное 'Betroffene' оставляй нейтральным как 'человек' или 'лицо', не превращая его в потерпевшего, преступника или подозреваемого. Сохраняй отрицание, правовой статус, субъект, объект и причинность.",
	}
	...
	gemma4LanguageGuidance = map[string]string{
		"es": "Use natural standard Spanish and output only the translation. In police-news context, German 'zuständige Dienststelle' means the relevant police station, police office, or police unit; translate it as 'dependencia policial competente' or equivalent, never as a service station or an unspecified office. German 'Scheibe' means generic vehicle glass or a window, so use 'cristal' or 'ventanilla' unless the source explicitly says 'Windschutzscheibe'; never infer 'parabrisas'. Preserve allegations, approximate times, actors, actions, negation, causality, medical meaning, and transit relationships exactly.",
		"it": "Usa un italiano standard naturale e restituisci soltanto la traduzione. Il tedesco 'Scheibe' indica genericamente un vetro o finestrino del veicolo: usa 'vetro' o 'finestrino' salvo che la fonte dica esplicitamente 'Windschutzscheibe'; non dedurre mai 'parabrezza'. Conserva la precisione temporale: 'um genau' significa 'esattamente alle', mai 'circa', mentre 'gegen' indica un orario approssimativo come 'verso' o 'circa'. Il neutro 'befragte' significa 'ha sentito' o 'ha intervistato', non 'ha interrogato', salvo che la fonte descriva esplicitamente un interrogatorio. Conserva esattamente accuse, incertezza, stato giuridico, negazioni, soggetti, azioni, causalità, significato medico e rapporti di trasporto.",
