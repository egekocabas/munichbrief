...
var structuredTranslationLanguageGuidance = map[string]string{
	"el": "Write entirely in standard Modern Greek except for immutable placeholders; never insert English or another language. Copy every placeholder occurrence exactly once and only where that same occurrence appears in its input field; never reuse a placeholder to translate nearby related words. Preserve reported allegations explicitly with standard Greek wording such as ‘φέρεται να’ plus a conjugated verb, never as established fact. Preserve approximate German times introduced by ‘gegen’ as approximate Greek times such as ‘περίπου στις’, never exact; do not invent ‘last’ for an unqualified weekday. German ‘später entlassen’ means neutrally released later, such as ‘αφέθηκε αργότερα ελεύθερος’, never the prison-specific ‘αποφυλακίστηκε’. German plural ‘S-Bahnen’ requires plural Greek grammar around its placeholder, such as ‘οι συρμοί … καθυστέρησαν’; never use singular ‘το τρένο … καθυστέρησε’. German ‘an der’ plus a STREET means on that street, so use concise ‘στην οδό’ wording, never ‘στην περιοχή της’. Keep headlines direct and concise without expanding a delayed vehicle into its departure. Use ‘τεκμήριο αθωότητας’ for the presumption of innocence and preserve ‘rechtskräftige Verurteilung’ as a final, legally binding conviction such as ‘τελεσίδικη καταδίκη’, not merely a judicial conviction. A past closure for a stated duration remains past, such as ‘παρέμεινε κλειστή’, not present ‘παραμένει’. German ‘befragte einen Zeugen’ means neutrally questioned a witness, such as ‘ρώτησε έναν μάρτυρα’, never investigated or arrested the witness.",
