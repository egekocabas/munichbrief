...
Tokens matching "__MB_PLACE_####__" stand for protected Munich-area place names. The same token can intentionally occur more than once for the same name. Copy every token occurrence exactly and keep it in the same output field, without translating, transliterating, splitting, or rewriting the token; natural target-language word order is allowed. A language may put an apostrophe-delimited grammatical suffix immediately after an intact token. Preserve Markdown syntax and link URLs exactly. Translate all other natural-language text.
