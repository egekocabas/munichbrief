...
func TestTimelineAndDetailRenderAIContentWithProvenance(t *testing.T) {
	...
	assistance, found, err := database.ClaimPostProcessingJob(context.Background(), processing.PublicAssistanceVerificationStep, false, nil, generatedAt.Add(time.Minute))
	if err != nil || !found {
