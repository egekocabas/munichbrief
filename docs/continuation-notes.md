...
| anschrift.net / COCENTER — service activation | **Confirmed in the operator-supplied order update of 14 September 2026, 20:19.** Identity verification is complete, the order is completed and the Bayern subscription is active. The supplied address matches the shared public operator details. | Retain the original evidence privately and keep the service active while publishing the address. No additional identity/activation confirmation request is needed. |
| anschrift.net / COCENTER — AVV | **Complete documents and conclusion process still pending.** The activation update does not supply the missing AVV annexes or establish account-specific acceptance. | Review the complete applicable AVV, security/subprocessor annexes and acceptance process when supplied. Do not resend the existing enquiry. |
